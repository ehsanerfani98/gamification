...
**آخرین به‌روزرسانی:** ۲۰۲۶-۰۹-۱۸ — Sprint 8: بخش «تنظیمات سایت» (فقط Admin) با دو حالت سندباکس (پیامک: کد OTP در پاسخ بدون ارسال واقعی؛ پرداخت: شبیه‌ساز داخلی با همان جریان callback/verify) + دستور `admin:promote` — **🚀 بتای ۵ فروشگاه شروع شد**؛ **۱۶۴ تست سبز (۲,۷۵۱ assertion)**
...
- [ ] ارتقای مدیر سایت روی سرور: `php artisan admin:promote` + ورود مجدد به پنل
- [ ] روشن‌کردن دو سندباکس از پنل و Onboarding ۵ فروشگاه پایلوت (ورود → فروشگاه → اشتراک سندباکس → کمپین → بازی → جایزه → کوپن → گزارش)
- [ ] جمع‌آوری بازخورد پایلوت‌ها و رفع اشکالات
