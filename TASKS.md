...
| Sprint 9 | پیکربندی درایورها از پنل | ۱ روز | ✅ تکمیل شد | ذخیره/بروزرسانی همه متغیرهای درگاه پرداخت و پیامک IPPanel از تنظیمات پنل + سندباکس رسمی ZarinPal (sandbox.zarinpal.com) |

**آخرین به‌روزرسانی:** ۲۰۲۶-۰۹-۱۸ — Sprint 9: پیکربندی درایور پیامک/پرداخت از پنل (اولویت DB بر env) + سندباکس رسمی زرین‌پال + نمایش کد OTP سندباکس در UI پنل و PWA — **۱۷۷ تست سبز (۲,۸۰۶ assertion)**
...
## Sprint 9 — پیکربندی درایورها از پنل + سندباکس رسمی ZarinPal ✅

### متغیرهای درگاه پرداخت و پیامک در تنظیمات سایت (درخواست کاربر)
- [x] Migration جدید: ۱۱ ستون nullable روی `site_settings` — مقدار null یعنی «پیش‌فرض env»؛ رفتار env-محور حفظ می‌شود
- [x] پیامک: `sms_channel` (log/ippanel)، `ippanel_api_key`، `ippanel_originator`، `ippanel_base_url`
- [x] پرداخت: `payment_gateway` (fake/zarinpal)، `zarinpal_merchant_id`، `zarinpal_sandbox`، `zarinpal_base_url`، `zarinpal_toman_to_rial`، `zarinpal_callback_url`، `zarinpal_description`
- [x] `SiteSettingsService`: resolverهای «DB مقدم بر env» برای همه متغیرها + مقاوم در برابر نبود جدول (حین migrate هرگز نمی‌شکند)
- [x] Bایند پویا در AppServiceProvider: سندباکس‌های داخلی → مقادیر پنل → env (ترتیب اولویت)
- [x] `GET/PATCH /api/v1/site-settings` — همه فیلدها قابل ذخیره/بروزرسانی؛ کلید API فقط ماسک‌شده (۴ کاراکتر آخر) برمی‌گردد و مقدار خالی = حفظ کلید فعلی؛ Audit بدون کلید خام
- [x] UI پنل: فرم کامل تنظیمات (سندباکس‌ها + بخش IPPanel + بخش ZarinPal با سوییچ سندباکس رسمی)
- [x] رفع رگرسیون `SiteSettings::current()`: پس از create اولین رکورد، رکورد دوباره خوانده می‌شود تا castهای boolean روی مقدار دیتابیس اجرا شوند (کرش `beta:status` روی مقدار null)
- [x] رکورد `payments.gateway` اکنون دروازه مؤثر (پنل/env) را ثبت می‌کند نه همیشه env را

### سندباکس رسمی ZarinPal (درخواست کاربر — «خود زرین‌پال حالت سندباکس دارد»)
- [x] کلید `zarinpal_sandbox` در تنظیمات: با روشن‌شدن، base URL به `sandbox.zarinpal.com` (همان API v4 با محیط آزمایشگاهی خود زرین‌پال) سوئیچ می‌شود
- [x] تست زنده مرورگری: Request → Authority واقعی سندباکس (`S000…8njwp6`) → ریدایرکت به صفحه پرداخت واقعی StartPay با کارت تست سندباکس → callback → وریفای واقعی (کد `-51` برای پرداخت ناتمام → failed صحیح)

### بهبود تجربه بتا
- [x] نمایش کد OTP سندباکس (`debug_code`) مستقیماً در UI ورود پنل و PWA مشتری (با کلیک = پرشدن خودکار فیلد) — قبلاً فقط در پاسخ API بود

### تست‌ها (۱۲ سناریوی جدید)
- [x] ذخیره/بروزرسانی همه متغیرهای IPPanel و ZarinPal از API پنل + ماسک کلید + حفظ کلید با ارسال خالی
- [x] اعتبارسنجی مقادیر نامعتبر درایور (422) + fallback به env با null
- [x] بایند پویا: درایور پیامک/دروازه از DB (بدون تغییر env) + تقدم سندباکس داخلی + پیش‌فرض fake/log
- [x] سندباکس رسمی زرین‌پال: base URL صحیح + ارسال request به sandbox.zarinpal.com با merchant از DB + وریفای با همان جریان v4
- [x] Audit بدون کلید خام — **مجموع ۱۷۷ تست سبز (۲,۸۰۶ assertion)**

### تست مرورگری کامل E2E (۱۰ بازی از ابتدا)
- [x] پنل: ورود OTP → ساخت کمپین تاس با Wizard → تعریف جایزه → Feature Gate سقف Plan (403 صحیح) → پرداخت سندباکس پایه → انتشار → پرداخت سندباکس حرفه‌ای
- [x] PWA: ورود OTP مشتری با نمایش کد سندباکس در UI → هر ۱۰ بازی اجرا و نتیجه گرفته شد: چرخ (برد کوپن ۱۰٪ با کد)، تاس، کارت خراش (حد نصاب ۳ نماد)، جعبه شانس، کارت شانس، بلیط شانس (کد GM…)، کوییز (۳/۳ قبول)، حافظه (برد ۱۰ امتیاز با شارژ Ledger)، واکنش (۳ دور + قضاوت سرور)، جرثقیل
- [x] کیف جایزه‌ها: کد کوپن با وضعیت + موجودی امتیاز Ledger
- [x] ثبت استفاده کوپن (ROI) در پنل + قیف آنالیتیکس کامل (View→Enter→Play→Win→Redeem)
- [x] چک‌این روزانه (+۱۰، Idempotent) + دعوت دوستان (پله ۱: +۱۰۰ امتیاز + Audit)
...
| تصمیم | وضعیت | توضیح |
|--------|--------|-------|
| سرویس پیامک OTP | ✅ نهایی شد — **IPPanel** | درایور `IppanelSmsChannel` آماده؛ فعال‌سازی با `SMS_CHANNEL=ippanel` + کلید API — از Sprint 9 همچنین قابل تنظیم از پنل (تنظیمات سایت) |
| دروازه پرداخت | ✅ نهایی شد — **ZarinPal** | درایور `ZarinpalGateway` آماده؛ فعال‌سازی با `PAYMENT_GATEWAY=zarinpal` + Merchant ID — از Sprint 9 قابل تنظیم از پنل + سندباکس رسمی `sandbox.zarinpal.com` |
...
| 15 | — | Sprint 9: ذخیره/بروزرسانی همه متغیرهای درگاه پرداخت و پیامک IPPanel از تنظیمات پنل (اولویت DB بر env) + سندباکس رسمی ZarinPal (sandbox.zarinpal.com) + نمایش کد OTP سندباکس در UI + رفع رگرسیون SiteSettings::current() (+۱۲ تست — **۱۷۷ تست سبز (۲,۸۰۶ assertion)**) | 2026-09-18 |
